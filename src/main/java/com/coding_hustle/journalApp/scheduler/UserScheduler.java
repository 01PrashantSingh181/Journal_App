...
import com.coding_hustle.journalApp.enums.Sentiment;
...
//    @Scheduled(cron = "0 0 9 ? * SUN *")
    public void fetchUsersAndSendSaMail() {
        ...
        for(User user:users){
            ...
            List<Sentiment> sentiments = journalEntries.stream().filter(x -> x.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime().isAfter(LocalDateTime.now().minus(7, ChronoUnit.DAYS))).map(x -> x.getSentiment()).collect(Collectors.toList());
